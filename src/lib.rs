...
//! `usize`, `bool`, ...). The bridge between `T` and its storage is the
//! [`AtomicRepr`] trait, which the [`impl_atomic_repr!`] macro implements for
//! `#[repr(uN)]` enums and other layout-compatible types.
//!
//! ```
//! # #![cfg_attr(feature = "nightly", feature(const_trait_impl))]
...
//! primitive integers and `bool` already do). For other types, the
...
//!   of `core::sync::atomic`, supporting targets without native atomics.
